...
import xyz.block.domainapi.kfsm.v2.util.HurdleGroupException.InvalidHurdleGroup
import xyz.block.domainapi.kfsm.v2.util.HurdleGroupException.UnknownHurdleGroup
...
interface InfoCollectionController<
  ...
  override fun processInputs(
    ...
    processInputsFromOperation(value, inputs, operation).bind()
      ?: if (inputs.all { it is Input.HurdleResponse }) {
        ...
        val failure = IllegalArgumentException("Inputs should be hurdle responses")
        raise(failure)
      }
  }
   ...
   */
  fun processInputsFromOperation(
    value: T,
    inputs: List<Input<R>>,
    operation: Operation
  ...
  fun transition(value: T): Result<T>
   ...
   */
  fun findMissingRequirements(value: T): Result<List<R>>
   ...
   */
  fun updateValue(value: T, hurdleResponse: Input.HurdleResponse<R>): Result<T>

  /** Called if the process is cancelled. */
  fun onCancel(value: T): Result<T>
   ...
   */
  fun getHurdlesForRequirementId(
    requirementId: R,
    value: T,
   ...
   */
  fun getFinalNotification(value: T): Result<UserInteraction.Notification<R>?> = null.success()
    ...
    if (missingRequirements.isEmpty()) {
      ...
      if (updatedMissingRequirements.isEmpty()) {
        ...
        ProcessingState.UserInteractions(
          hurdles = hurdles,
          nextEndpoint = if (hurdles.any { requiresSecureEndpoint(it.id) }) {
  ...
  private fun mergeHurdles(current: List<Hurdle<R>>, updates: List<Hurdle<R>>): List<Hurdle<R>> {
    val updateMap = updates.associateBy { it.id }
    val updatedCurrent = current.map { hurdle ->
  ...
  fun requiresSecureEndpoint(requirement: R): Boolean

  fun goBack(value: T, hurdleResponse: Input.HurdleResponse<R>): Result<T> =
  ...
  private fun checkCancelled(value: T, requirementResults: List<Input<R>>): Result<Unit> = result {
    requirementResults
    ...
    hurdleResponses
      .fold(Pair(missingRequirements, value).success()) {
        ...
        if (currentMissingRequirements.contains(hurdleResponse.id)) {
          ...
          val updatedValue = when (hurdleResponse.result) {
          ...
          val failure = DomainApiError.InvalidRequirementResult(
            value.id.toString(),
