...
abstract class InfoCollectionController<
  ...
  override fun process(value: T, inputs: List<Input<R>>): Result<ProcessingState<T, R>> = result {
   ...
   */
  abstract fun updateValue(value: T, hurdleResponse: Input.HurdleResponse<R>): Result<T>

  /** Called if the process is cancelled. */
  abstract fun onCancel(value: T, ctx: DSLContext? = null): Result<T>

  /**
   * Returns a hurdle for the given requirement ID.
   *
   * @param requirementId The ID of the requirement.
   * @param value The process value.
   * @return A hurdle for the given requirement ID.
   */
  abstract fun getHurdleForRequirementId(
    requirementId: R,
    value: T
  ): Result<UserInteraction.Hurdle<R>?>
  ...
  private fun processPendingCollectionState(
    ...
    if (missingRequirements.isEmpty()) {
      ...
      if (updatedMissingRequirements.isEmpty()) {
        ...
        val hurdles = updatedMissingRequirements.mapNotNull {
          getHurdleForRequirementId(it, value).bind()
        }
      ...
      .fold(Pair(missingRequirements, value).success()) {
        ...
        if (currentMissingRequirements.contains(hurdleResponse.id)) {
          ...
          val updatedValue = when (hurdleResponse.result) {
            ResultCode.CLEARED, ResultCode.FAILED -> updateValue(
              currentValue,
              hurdleResponse,
            ).bind()
            ResultCode.SKIPPED -> currentValue
            else -> raise(
              IllegalArgumentException("Unexpected hurdle response code ${hurdleResponse.result}")
            )
          }
